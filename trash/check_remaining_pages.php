...
// إجمالي الصفحات
$result = $mysqli->query("SELECT COUNT(*) as total FROM pages");
$total = $result->fetch_assoc()['total'];
echo "📊 إجمالي الصفحات في MySQL: " . number_format($total) . "\n";

// أعلى ID
$result = $mysqli->query("SELECT MAX(id) as max_id FROM pages");
$max_id = $result->fetch_assoc()['max_id'];
echo "🔝 أعلى ID: " . number_format($max_id) . "\n";

// الصفحات بعد 5855265
$result = $mysqli->query("SELECT COUNT(*) as remaining FROM pages WHERE id > 5855265");
$remaining = $result->fetch_assoc()['remaining'];
echo "📄 الصفحات المتبقية (ID > 5855265): " . number_format($remaining) . "\n";

if ($remaining > 0) {
    // أول صفحة بعد 5855265
    $result = $mysqli->query("SELECT id, book_id, page_number FROM pages WHERE id > 5855265 ORDER BY id ASC LIMIT 1");
    ...
    // آخر صفحة
    $result = $mysqli->query("SELECT id, book_id, page_number FROM pages WHERE id > 5855265 ORDER BY id DESC LIMIT 1");
