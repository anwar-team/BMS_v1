    ...
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                raise ShamelaScraperError(f"فشل في الوصول إلى {url} بعد {retries} محاولات")
    ...
    for page_num in range(start_page, end_page + 1):
        try:
            ...
            if page_num % 10 == 0 or page_num == end_page:
                ...
                logger.info(f"تم استخراج {page_num - start_page + 1} من {total_pages} صفحة ({progress:.1f}%)")
            
        except Exception as e:
            logger.error(f"خطأ في استخراج الصفحة {page_num}: {e}")
            # إضافة صفحة فارغة للحفاظ على الترقيم
            empty_page = PageContent(
                page_number=page_num,
                content=f"خطأ في استخراج الصفحة: {e}",
                word_count=0
            )
            pages.append(empty_page)
    
    logger.info(f"تم استخراج {len(pages)} صفحة من الكتاب {book.shamela_id}")
    return pages
