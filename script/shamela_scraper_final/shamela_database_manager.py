...
class ShamelaDatabaseManager:
    ...
    def close(self) -> None:
        """إغلاق الاتصال بقاعدة البيانات (مرادف لـ disconnect)"""
        self.disconnect()
    ...
    def save_chapter(self, chapter: Chapter, book_id: int, volume_id: Optional[int] = None, 
        ...
        query = f"""
            ...
            WHERE book_id = %s AND title = %s AND page_start = %s LIMIT 1
        """
        ...
        if result:
            ...
            insert_query = f"""
                ...
                (book_id, volume_id, title, page_start, page_end, parent_id, level, created_at, updated_at)     
