        ...
        <div class="py-1" role="none">
            @foreach($languages as $code => $language)
                <a href="{{ route('language.switch', $code) }}" 
                   @click="open = false"
                   class="group flex items-center justify-between w-full px-4 py-3 text-left text-sm transition-colors duration-150 hover:bg-gray-100 dark:hover:bg-gray-800 {{ $currentLocale === $code ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/20 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300' }}" 
