...
echo "========================================\n";
echo "اختبار استخراج ومطابقة المؤلفين\n";
echo "========================================\n\n";

// إنشاء instance من الـ Extractor
$extractor = new AuthorExtractor();
...
foreach ($books as $index => $book) {
    ...
    $relevantLines = array_filter($descLines, function($line) {
        return mb_stripos($line, 'المؤلف') !== false 
            || mb_stripos($line, 'المصنف') !== false
            || mb_stripos($line, 'الكاتب') !== false;
    });
    
    if (!empty($relevantLines)) {
        echo "\nالسطر المتعلق بالمؤلف:\n";
    ...
    try {
        ...
        if (!$result) {
            echo "❌ لم يتم استخراج مؤلف من الوصف\n\n";
            continue;
        }
        
        $stats['extracted']++;
        
        echo "📋 المؤلف المستخرج: \"{$result['extracted_author_name']}\"\n";
        
        if ($result['matched_author_id']) {
            $stats['matched']++;
            
            $matchDetails = $result['match_details'];
            $confidence = $result['author_match_confidence'];
            ...
            echo "   - المؤلف المطابق: {$matchDetails['author_name']}\n";
            ...
            echo "   - يحتاج إنشاء مؤلف جديد أو مراجعة يدوية\n";
        }
...
echo "تم استخراج مؤلف: {$stats['extracted']} (" . round(($stats['extracted'] / $stats['total']) * 100, 1) . "%)\n";
