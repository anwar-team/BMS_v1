    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->filters([
                ...
                Filter::make('show_similar_books')
                    ->form([
                        Select::make('similarity_threshold')
                            ->label('مستوى التشابه المطلوب')
                            ->options([
                                'high' => 'عالي - كتب متطابقة تقريباً',
                                'medium' => 'متوسط - كتب متشابهة بشكل واضح',
                                'low' => 'منخفض - كتب لها تشابه جزئي',
                            ])
                            ->default('medium')
                            ->helperText('اختر مستوى التشابه لعرض الكتب المتشابهة مع بعضها البعض'),
                        
                        Toggle::make('group_by_similarity')
                            ->label('ترتيب حسب التشابه')
                            ->default(true)
                            ->helperText('عرض الكتب مرتبة حسب درجة التشابه'),
                    ])
                    ->query(function (Builder $query, array $data): Builder {
                        $threshold = $data['similarity_threshold'] ?? 'medium';
                        ...
                        return $query->whereExists(function ($subQuery) use ($threshold) {
                            $subQuery->select(\Illuminate\Support\Facades\DB::raw(1))
                                ...
                                ->where(function ($similarityQuery) use ($threshold) {
                                    switch ($threshold) {
                                        case 'high':
                                            // تشابه عالي: نفس الكلمات الأساسية
                                            $similarityQuery->whereRaw("
                                                MATCH(similar_books.title) AGAINST(
                                                    CONCAT('\"', SUBSTRING_INDEX(books.title, ' ', 3), '\"') 
                                                    IN BOOLEAN MODE
                                                ) > 0.5
                                                OR (
                                                    LENGTH(books.title) > 10 AND 
                                                    LENGTH(similar_books.title) > 10 AND
                                                    SOUNDEX(SUBSTRING(books.title, 1, 20)) = SOUNDEX(SUBSTRING(similar_books.title, 1, 20))
                                                )
                                            ");
                                            break;
                                            
                                        case 'medium':
                                            // تشابه متوسط: كلمات مشتركة مهمة
                                            $similarityQuery->whereRaw("
                                                (
                                                    books.title LIKE CONCAT('%', SUBSTRING_INDEX(similar_books.title, ' ', 2), '%')
                                                    OR similar_books.title LIKE CONCAT('%', SUBSTRING_INDEX(books.title, ' ', 2), '%')
                                                )
                                                AND LENGTH(books.title) > 5 
                                                AND LENGTH(similar_books.title) > 5
                                            ");
                                            break;
                                            
                                        case 'low':
                                            // تشابه منخفض: أي كلمة مشتركة مهمة
                                            $similarityQuery->whereRaw("
                                                (
                                                    books.title LIKE CONCAT('%', SUBSTRING_INDEX(similar_books.title, ' ', 1), '%')
                                                    OR similar_books.title LIKE CONCAT('%', SUBSTRING_INDEX(books.title, ' ', 1), '%')
                                                )
                                                AND LENGTH(SUBSTRING_INDEX(books.title, ' ', 1)) > 3
                                                AND LENGTH(SUBSTRING_INDEX(similar_books.title, ' ', 1)) > 3
                                            ");
                                            break;
                                    }
                                });
                    ...
                    ->indicateUsing(function (array $data): ?string {
                        if ($data['similarity_threshold']) {
                            $level = match($data['similarity_threshold']) {
                                'high' => 'عالي',
                                'medium' => 'متوسط',
                                'low' => 'منخفض',
                                default => 'متوسط'
                            };
                            return "عرض الكتب المتشابهة (مستوى: {$level})";
                        }
                        return null;
                    }),
